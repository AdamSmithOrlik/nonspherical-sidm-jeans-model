...
def relaxation(r1, outer_halo, L_list=[0], M_list=[0], converge=None, 
               max_iter=20, init_grid=5, r_grid=200, verbose=False, finaltol=1e-8, **extraneous):
    ...
    profile, success_flag = jeans.spherical.relaxation(r1, outer_halo, init_grid=init_grid, max_iter=max_iter,
                                                       r_grid=r_grid, verbose=verbose, finaltol=finaltol)
    ...
    # Step 3: Check convergence
        ...
        while not(convergence_test) and success_flag:
            ...
            if verbose:
                print('  Convergence test with increasing L was a success:', convergence_test)
...
def F(profile, i):
    ...
    if (num_variables == 1) & (profile.L_list == [0]):
        ...
        if profile.angular_moments_set:
            ...
            term_2 = - r_bar**2/r0**2 * np.array([ 2*np.pi * integrate( integrand, 0, np.pi, args=[L]) for L in L_list ])
        ...
        print('Computing F with', num_variables, 'coordinates not supported. Returning zero.')
...
def A_matrix(profile, i):
        ...
        for a in range(num_size):
            for b in range(a,num_size):
                ...
                if profile.angular_moments_set and (a==0):
                    dFdphi[a,b] = r_bar**2/r0**2 * 4*np.pi * Z(0,0,0,0) * profile.angular_moments(r_bar,L_list[b])

                else:
                    integrand = lambda th: Z(L_list[a],0,th,0)*Z(L_list[b],0,th,0)*np.exp( - phi_b - phi_dm(th) )*np.sin(th)
    ...
    if num_variables == 2:
        ...
        print('Computing A with', num_variables, 'coordinates not supported. Returning zero.')
...
def B_matrix(profile, i):
    ...
    if (num_variables == 1) & (profile.L_list == [0]):
        ...
        print('Computing B with', num_variables, 'coordinates not supported. Returning zero.')
...
def M_matrix(profile, outer_halo):
    ...
    M_out[row_num:row_num+num_func+num_params,((num_r-1)*num_func):(num_r*num_func)] = CN
