...
def Z(L,M,theta,phi): 
    ...
    if M == 0:
        ...
        return Condon_Shortley_phase * np.sqrt(2*(2*L+1)/(4*np.pi)) * np.sqrt(fact(L-M)/fact(L+M)) * lpmv(M,L,x) * np.cos(M*phi)
    elif M < 0:
        fact = lambda num: factorial(num,exact=True)
        return (-1) * Condon_Shortley_phase * np.sqrt(2*(2*L+1)/(4*np.pi)) * np.sqrt(fact(L+M)/fact(L-M)) * lpmv(-M,L,x) * np.sin(M*phi)
    else:
        return 0
