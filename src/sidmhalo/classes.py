...
from sidmhalo.cdm import AC_profiles, NFW_profiles, mass_concentration_to_NFW_parameters, NFW_parameters_to_mass_concentration
...
class profile:
    ...
    def q_eff(self,r_sph,qmax=1000,qmin=0.001):
        ...
        def find_q(r):
            ...
            f = lambda q: np.log(rho(r*q**(-1/3.),0)/rho(0,r*q**(2/3.)))
            try:
    ...
    def q_shell(self,r_eff,qmax=2,qmin=0.5,method='brentq',maxiter=100,rtol=1e-3,atol=1e-3,verbose=False):
    
        if np.ndim(r_eff) == 0:

            rho = self.rho_sph
            r = lambda th,q: r_eff/np.sqrt( (np.sin(th)*q**(2/3))**2 + (np.cos(th)*q**(-1/3))**2)
            ...
            if method == 'iterate':
                ...
                while num_iter < maxiter:
                    ...
                    integrand = lambda th: 2 * np.sin(th) * np.cos(th)**2 * rho(r(th,q_old),th) * r(th,q_old)**4
                    I_zz = integrate(integrand,0,np.pi)

                    integrand = lambda th: np.sin(th) * np.sin(th)**2 * rho(r(th,q_old),th) * r(th,q_old)**4
                ...
                try:
                    ...
                    output = self.q_shell(r_eff,qmax=2*qmax,qmin=0.5*qmin,atol=atol,rtol=rtol,method=method)
            ...
            return np.array([self.q_shell(ri,method=method,qmax=qmax,qmin=qmin,maxiter=maxiter,atol=atol,rtol=rtol) for ri in r_eff])

        else:
            raise Exception("r must be 0D or 1D.")
    
    def q_encl(self,r_eff,qmax=2,qmin=0.5,q_init=1,method='brentq',maxiter=100,rtol=1e-3,atol=1e-3,epsabs=1e-5,epsrel=1e-5,verbose=False):
    
        if np.ndim(r_eff) == 0:
            #print('r_eff: ', r_eff)

            rho = self.rho_dm_sph
            r_spheroid = lambda th,q: r_eff/np.sqrt( (np.sin(th)*q**(2/3))**2 + (np.cos(th)*q**(-1/3))**2)
            r_min = lambda th: 0
            ...
            if method == 'iterate':
                ...
                try:
                    ...
                    output = self.q_encl(r_eff,qmax=2*qmax,qmin=0.5*qmin,atol=atol,rtol=rtol,method=method)
            ...
            return np.array([self.q_encl(ri,method=method,qmax=qmax,qmin=qmin,maxiter=maxiter,atol=atol,rtol=rtol) for ri in r_eff])
...
class isothermal_profile:
    ...
    def update_angular_moments(self):
        ...
        phi_list = np.array([ self.y[i*self.num_func:i*self.num_func+num_size] for i in range(self.num_r_points) ])
    ...
    def update(self,**updated_inputs):
        ...
        for key, value in updated_inputs.items():
            ...
            if key == 'params': self.params = value
            if key == 'Phi_b': self.Phi_b = value
    ...
    def copy(self):
        return isothermal_profile(self.y, self.params, self.r_list, Phi_b=self.Phi_b, L_list=self.L_list, M_list=self.M_list)

    # Makes dictionary of outputs that can be saved
    def tosave(self,**extra):
        return {'y':self.y, 'params':self.params, 'r_list':self.r_list, 'L_list':self.L_list, 'M_list':self.M_list}
...
class CDM_profile:
    ...
    def __init__(self,*inputs,q0=1,M_b=None,Phi_b=None,AC_prescription=None,Gnedin_params=(1.6,0.8),input_NFW=False,**extraneous):
        ...
        # AC_prescription = 'Gnedin' or 'Cautun'
            ...
            rho, M_encl = AC_profiles(M200,c,self.M_b,AC_prescription=AC_prescription,Gnedin_params=Gnedin_params)
    ...
    def rho_cyl(self,R,z):
        # spheroidal r
        r_sph = np.sqrt((R * self.q0**(1/3.))**2 + (z * self.q0**(-2/3.))**2)
    ...
    def tosave(self):
        ...
        outputs = {'M200':self.M200,'c':self.c,'q0':self.q0,'AC_prescription':self.AC_prescription}
...
def load_profile(file_name, Phi_b=no_baryons, M_b=None, verbose=False):
    ...
    try:
        ...
        try:
            ...
            outer_halo = CDM_profile(M200,c,q0=q0,Phi_b=Phi_b,AC_prescription=AC_prescription,Gnedin_params=Gnedin_params,M_b=M_b)
        ...
        profile = isothermal_profile(y=y, params=params, r_list=r_list, L_list=L_list, M_list=M_list, Phi_b=Phi_b)
        success_test = True
